...
ui<-fluidPage(
  ...
  headerPanel("Tweet Based Intelligent Movie Marketing "),
  ...
  tabsetPanel(
    ...
    tabPanel("Suggested number of screens",textOutput("noOfScreens"),width=10),
    tabPanel("Top Users",tableOutput("topUsers"),width=10)
  ),
...
server<-function(input, output)
{
  ...
  key <- "yg3NK1BMfLx6dEE7UenZMMGiW" 
  secret <- "MFwnagcHiyQMvkETkiZ613ngiWkKAKXN0lHVq6bW4g687G20R9" 
  ...
  output$noOfScreens<-renderText({captainamericatweets = searchTwitter("Captain America",n = 100, lang = "en",geocode=input$select)
  head(captainamericatweets)
  ...
  output$topUsers<-renderTable({captainamericatweets = searchTwitter("Captain America",n = 100, lang = "en",geocode=input$select)
  head(captainamericatweets)
  
  captainamericalistnames <- sapply(captainamericatweets,function(x) x$getScreenName())
  #using function to get usernames of of owner of the tweets
  #Scraping user details
  users <- lookupUsers(captainamericalistnames)
  ...
  a <- sortedusers[1:10,]
  top10Users <- a$screenName
  #location<-a$location
  followersCount<-a$followersCount
  top10Users<-cbind(top10Users,followersCount)
  top10Users<-as.data.frame(top10Users)
  write.csv(top10Users,file="top10Users.csv")
  top10Users
